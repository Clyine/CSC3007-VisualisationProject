...
include_graphics("previous_visualisation.jpg")
```
...
### Step 3: Get Oil Production Split by BRIC and Non-BRIC
...
### Step 4: Write CSV Out for BRIC and Non-BRIC Oil Data
...
## Total Oil and Car Production of BRIC and Non-BRIC Countries
```{r}
## Total Oil and Car Production of BRIC and Non-BRIC Countries
total_oil = total_oil_grouped |>
  pivot_longer(cols = !c(Country), names_to = "Year", values_to = "Total") |>
  group_by(Year) |>
...
print(total_oil)
  ...
  ggplot() +
  geom_area(data=total_car, aes(x=Year, y=Cars, fill=Country, group=Country), alpha = 0.5) +
  geom_line(data=total_oil, aes(x=Year, y=Sum*1500, group=1), size=1) +
  labs(title = "Total Car Production",
       subtitle = "By BRIC & non-BRIC Countries and Year",
       x = "Year",
       y = "Total Car Production (Millions/M)",
       caption = "Source: EIA") +
  scale_y_continuous(labels = unit_format(unit = "M", scale = 1e-6),
                     sec.axis = sec_axis(~ . / 1500, name = "Oil (scaled)")) +
...
## Total Cars Production of All Countries
...
## Step 1: Extract and Format World Oil Supplies

```{r}
# Extract "WORLD" row from new_oil_df
world_row <- new_oil_df[new_oil_df$Country == "WORLD", ]

# Remove the first column (Country)
world_data <- as.numeric(world_row[-1])

# Create a data frame with "Oil Supplies" as Country and the extracted data
world_df <- data.frame(Country = "Oil Supplies", t(world_data))

print(world_df)
```

## Step 2: Get Car Production by BRIC and Non-BRIC
...
## Step 3: Verify both dataframes have different columns based on years

```{r}
colnames(processed_cars_grouped)
colnames(world_df)
```

## Step 4: Create dataframe with desired years for World Supplies

```{r} 
years <- as.character(1999:2023)
template <- data.frame(matrix(NA, nrow = 1, ncol = length(years) + 1))
colnames(template) <- c("Country", years)
```

## Step 5: Extract world data and select columns for 1999 to 2023

```{r}
world_selected <- new_oil_df %>%
  filter(Country == "WORLD") %>%
  select(Country, all_of(years))

print(world_selected)
```

## Step 6: Plot Car Production and Oil Supplies
```{r}
world_long <- world_selected %>%
  pivot_longer(-Country, names_to = "Year", values_to = "Oil_Supplies")

cars_long <- processed_cars_grouped %>%
  pivot_longer(-Country, names_to = "Year", values_to = "Car_Production")

# Normalize the data
world_long <- world_long %>%
  mutate(Normalized_Oil_Supplies = Oil_Supplies / max(Oil_Supplies))

cars_long <- cars_long %>%
  mutate(Normalized_Car_Production = Car_Production / max(Car_Production))

# Combine the normalized data
combined_normalized <- bind_rows(
  world_long %>% rename(Value = Normalized_Oil_Supplies) %>% mutate(Type = "Normalized Oil Supplies"),
  cars_long %>% rename(Value = Normalized_Car_Production) %>% mutate(Type = "Normalized Car Production")
)


# Plot
ggplot() +
  geom_ribbon(data = world_long[world_long$Country == "WORLD", ], aes(x = as.numeric(Year), ymin = 0, ymax = Normalized_Oil_Supplies, fill = Country), alpha = 0.2) +
  geom_ribbon(data = world_long[world_long$Country == "non-BRIC", ], aes(x = as.numeric(Year), ymin = 0, ymax = Normalized_Oil_Supplies, fill = Country), alpha = 0.2) +
  geom_ribbon(data = world_long[world_long$Country == "BRIC", ], aes(x = as.numeric(Year), ymin = 0, ymax = Normalized_Oil_Supplies, fill = Country), alpha = 0.2) +

  geom_ribbon(data = cars_long[cars_long$Country == "WORLD", ], aes(x = as.numeric(Year), ymin = 0, ymax = Normalized_Car_Production, fill = Country), alpha = 0.2) +
  geom_ribbon(data = cars_long[cars_long$Country == "non-BRIC", ], aes(x = as.numeric(Year), ymin = 0, ymax = Normalized_Car_Production, fill = Country), alpha = 0.2) +
  geom_ribbon(data = cars_long[cars_long$Country == "BRIC", ], aes(x = as.numeric(Year), ymin = 0, ymax = Normalized_Car_Production, fill = Country), alpha = 0.2) +

  geom_line(data = world_long, aes(x = as.numeric(Year), y = Normalized_Oil_Supplies, color = Country, linetype = "Oil Supplies"), linewidth = 1) +
  geom_line(data = cars_long, aes(x = as.numeric(Year), y = Normalized_Car_Production, color = Country, linetype = "Car Production"), linewidth = 1) +
  
  scale_y_continuous(
    name = "Normalized Values",
    sec.axis = sec_axis(~ . * max(cars_long$Car_Production), name = "Car Production")
  ) +
  labs(
    title = "Normalized World Oil Supplies and Car Production (BRIC and Non-BRIC)",
    x = "Year",
    color = "Country",
    linetype = "Type",
    fill = "Country",
    caption = "Source: OICA, EIA"
  ) +
  theme_minimal()
```


## Percentage Change in Oil Production from Baseline

```{r}
# Define the baseline year
base_year <- min(common_years)

# Calculate percentage change for car production
total_oil_grouped_pct_change <- total_oil_grouped %>%
  pivot_longer(cols = -Country, names_to = "Year", values_to = "Total_petroleum_and_other_liquids") %>%
  mutate(Year = as.numeric(Year)) %>%
  group_by(Country) %>%
  mutate(Percent_Change = (Total_petroleum_and_other_liquids - Total_petroleum_and_other_liquids[Year == base_year]) / Total_petroleum_and_other_liquids[Year == base_year] * 100)

# Calculate percentage change for world oil supplies
world_selected_long <- world_selected %>%
  pivot_longer(cols = -Country, names_to = "Year", values_to = "World_Supplies") %>%
  mutate(Year = as.numeric(Year)) %>%
  group_by(Country) %>%
  mutate(Percent_Change = (World_Supplies - World_Supplies[Year == base_year]) / World_Supplies[Year == base_year] * 100)

# Combine the data frames
combined_pct_change <- bind_rows(
  total_oil_grouped_pct_change %>% select(Country, Year, Percent_Change),
  world_selected_long %>% select(Country, Year, Percent_Change)
)

# Plot the combined data
combined_pct_change %>%
  ggplot(aes(x = Year, y = Percent_Change, color = Country, group = Country)) +
  geom_line(linewidth = 1) +
  labs(
    title = "Percentage Change in Total Petroleum and Other Liquids Production",
    subtitle = paste("From Baseline Year", base_year),
    x = "Year",
    y = "Percentage Change (%)",
    caption = "Source: EIA"
  ) +
